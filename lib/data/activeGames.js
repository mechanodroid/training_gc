...
function loadMasterList(callback){
  var feather = this.feather;
    feather.logger.info({message:'Getting master games list from server',category:'activegames.api'});
    feather.data.appdb.view("thing:game/default", { descending: true }, function(err, dbResult) {
      if (!err) {
        feather.logger.info({message:'Found ' + dbResult.length + ' games.', category:'activegames.api'});
        dbResult.forEach(function(key, doc, id) {
          doc.key = key;
          doc.id = id;
          feather.logger.debug({message:'Found document w/ id ' + id + ', key ' + key, category:'activegames.api'});
        });
      } else {
        feather.logger.error({message:"Error getting results from couch", exception:err, category:'activegames.api'});
      }
      if (callback) {
